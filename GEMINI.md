...
キーボードショートカットとアクションの管理:

アクション定義 (src/actions.rs):
- アプリケーション全体で使用されるアクション（OpenFile, Rename, SelectItem, OpenFolder, CloseFolder, AddEditorPanel など）を定義する。
- 各アクションは Action トレイトを実装し、必要に応じて JsonSchema と Deserialize を derive する。
- アクションは単一責任原則に従い、明確な目的を持つこと。

グローバル状態管理 (src/app_state.rs):
- AppState 構造体でアプリケーション全体の状態を管理する。
- EditorService など、複数のコンポーネントから参照される共有サービスを保持する。
- Global トレイトを実装し、cx.global::<AppState>() でアクセス可能にする。

キーバインディング登録 (src/keybindings.rs):
- init() 関数でアプリケーション全体のキーバインディングを登録する。
- KeyBinding::new() を使用して、キーストロークとアクションを紐付ける。
- 例: Shift+Escape → ToggleZoom, Ctrl+W → ClosePanel

アクションのハンドリング:
- その責務を持つ適切な View または Model で .on_action() を使用してハンドリングする。
- 例: OpenFile は WindowRoot、MoveCursor は EditorView で処理する。
- cx.listener() を使用して、エンティティの状態を更新するハンドラを登録する。